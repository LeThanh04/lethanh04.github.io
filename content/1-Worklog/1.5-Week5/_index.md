---
title: "Week 5 Worklog"
date: 2026-05-18
weight: 5
chapter: false
pre: " <b> 1.5. </b> "
---


### Week 5 Objectives:

* Understand Amazon Simple Storage Service (Amazon S3) and its role in data storage on the AWS platform.

### Tasks to be carried out this week:
| Day | Task                                                                                                                                                                                                   | Start Date | Completion Date | Reference Material                        |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------- | --------------- | ----------------------------------------- |
| 2   | - An overview of Amazon S3 and the service's components                                                                                         | 05/18/2026 | 05/18/2026 |
<https://cloudjourney.awsstudygroup.com/> |
| 3   | - Understanding Bucket Policies, Access Control, and Data Access Permissions                                                                                        | 05/19/2026 | 05/19/2026 | <https://cloudjourney.awsstudygroup.com/> |
| 4   | - Explore the integration capabilities between Amazon S3 and other AWS services            | 05/20/2026 | 05/20/2026 |<https://cloudjourney.awsstudygroup.com/> |
| 5   | - **Practice:** <br>&emsp; + Create S3 Bucket <br>&emsp; + Configure access permissions (Bucket Policy)                                                                                            | 05/21/2026 | 05/22/2026 | <https://cloudjourney.awsstudygroup.com/> |
| 6   | - **Practice:** <br>&emsp; + Practice enabling Versioning and explore Amazon S3 storage classes.                                                                                           | 05/23/2026 | 05/23/2026 |<https://cloudjourney.awsstudygroup.com/> |


### Week 5 Achievements:

* Gain an understanding of the Amazon S3 service and its role in data storage and management on the AWS platform.

* Understand the operating principles of Bucket Policies and access control mechanisms.

* Grasp the integration capabilities of Amazon S3 with other AWS services.
